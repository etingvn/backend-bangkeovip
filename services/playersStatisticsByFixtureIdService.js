const PlayersStatisticsByFixtureId = require('../models/playersstatisticsbyfixtureidModel');
const axios = require('axios');
const url = require('url');
const Redis = require('ioredis');

class PlayersStatisticsByFixtureIdService {
    constructor() {
    ...
    playersStatisticsByFixtureId = async (query) => {
        ...
        const someAsyncFunction = async (item, parameters) => {
            ...
            const dynamicDocument = new PlayersStatisticsByFixtureId({ parameters: parameters, data: item});
        ...
        try {
            ...
            const query = { 'parameters.fixture': queryObject.fixture };
            const keysCache = await redis.keys(`PlayersStatisticsByFixtureId:${queryObject.fixture}:*`);
            if (keysCache.length === 0) {
                // Check amountCountDb ở MongoDB
                const amountCountDb = await PlayersStatisticsByFixtureId.countDocuments(query).exec();
                if (amountCountDb === 0) {
                    ...
                    let documents = await PlayersStatisticsByFixtureId.find(query);
                    await documents.forEach((doc, index) => {
                        ...
                        redis.set(`PlayersStatisticsByFixtureId:${queryObject.fixture}:${index}`, jsonString, (err) => {
                            if (err) {
...
module.exports = PlayersStatisticsByFixtureIdService;
