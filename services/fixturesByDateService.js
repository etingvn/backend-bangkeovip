const Fixturesbydate = require('../models/fixturesbydateModel');
...
class FixturesByDateService {
    ...
    fixturesByDate = async (query) => {
        ...
        const someAsyncFunction = async (item, parameters) => {
            ...
            const dynamicDocument = new Fixturesbydate({ parameters: parameters, data: item});
        ...
        try {
            ...
            const keysCache = await redis.keys(`Fixtureheadtohead:${queryObject.date}:*`);
            if (keysCache.length === 0) {
                // Check amountCountDb ở MongoDB
                const amountCountDb = await Fixturesbydate.countDocuments(query).exec();
                if (amountCountDb === 0) {
                    ...
                    let documents = await Fixturesbydate.find(query);
                    await documents.forEach((doc, index) => {
                        ...
                        redis.set(`Fixturesbydate:${queryObject.date}:${index}`, jsonString, (err) => {
                            if (err) {
                    ...
                    const keyList = await redis.keys(`Fixturesbydate:${queryObject.date}:*`);
...
module.exports = FixturesByDateService;
